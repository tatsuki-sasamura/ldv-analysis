...
for npz_path in npz_files:
    ...
    h_freqs = drive_freqs[:, None] * harmonics_mul[None, :]  # (n_points, MAX_HARMONIC)
    ...
    spectrum = np.abs(fft_vel) * 2 / n_samples * VELOCITY_SCALE  # (n_points, n_freq_bins)
    ...
    phase_vel_h = np.angle(fft_vel[pts[:, None], h_idx])  # (n_points, MAX_HARMONIC)
    diff = np.degrees(phase_vel_h) - np.degrees(phase_v1f[:, None]) * harmonics_mul[None, :]
    all_phases = (diff + 180) % 360 - 180

    # Ch3 displacement amplitude at each harmonic
    spectrum_disp = np.abs(fft_disp) * 2 / n_samples * DISPLACEMENT_SCALE  # (n_points, n_freq_bins) in m
    ...
    print(f"  RMS vel:    " + ", ".join(f"{l}={v:.2e}" for l, v in zip(labels, rms)))
    print(f"  RMS p(Ch2): " + ", ".join(f"{l}={v:.1f} kPa" for l, v in zip(labels, rms_prs_ch2 / 1e3)))
    print(f"  RMS p(Ch3): " + ", ".join(f"{l}={v:.1f} kPa" for l, v in zip(labels, rms_prs_ch3 / 1e3)))
...
ax.set_title("Harmonic Content — RMS Across Scan Points (Ch2, refracto-vibrometry)")
...
ax.set_xlabel("Harmonic")
ax.set_title("Harmonic Ratio Relative to Fundamental — Apparent Velocity (RMS)")
ax.legend(fontsize=7)
ax.grid(True, alpha=0.3, axis="y")
plt.tight_layout()
...
for j, stem in enumerate(stems):
    ...
    offset = (j - n_files / 2 + 0.5) * width
    if rms_prs_ch2[0] > 0:
        axes[0].bar(x + offset, rms_prs_ch2 / rms_prs_ch2[0], width, label=stem)
    if rms_prs_ch3[0] > 0:
        axes[1].bar(x + offset, rms_prs_ch3 / rms_prs_ch3[0], width, label=stem)
...
for stem, (pos_x, all_harmonics, all_phases, prs_ch2, prs_ch3, rssi) in file_harmonics_per_point.items():
    ...
    # Apparent velocity
    for h in range(MAX_HARMONIC_PROFILE):
        axes[0].plot(pos_x[order], all_harmonics[order, h],
                     label=f"{h+1}f", marker=".", markersize=2, linewidth=0.8)
    ...
    for h in range(MAX_HARMONIC_PROFILE):
        axes[1].plot(pos_x[order], prs_ch2[order, h] / 1e3,
                     label=f"{h+1}f", marker=".", markersize=2, linewidth=0.8)
    ...
    for h in range(MAX_HARMONIC_PROFILE):
        axes[2].plot(pos_x[order], prs_ch3[order, h] / 1e3,
                     label=f"{h+1}f", marker=".", markersize=2, linewidth=0.8)
    ...
    # Phase
    for h in range(MAX_HARMONIC_PROFILE):
        axes[3].plot(pos_x[order], all_phases[order, h],
                     label=f"{h+1}f", marker=".", markersize=2, linewidth=0.8)
...
for stem, rep in file_representative.items():
    ...
    fft_prs_complex[1:] = fft_vel_complex[1:] / (2 * np.pi * freqs[1:] * SENSITIVITY)
    ...
    axes[0, 1].set_title("Pressure (converted from Ch2)")
    ...
    for h in range(MAX_HARMONIC):
        ...
        axes[1, 0].text(h_freq / 1e6, fft_spec.max() * 0.9, f" {h+1}f",
    ...
    for h in range(MAX_HARMONIC):
        ...
        axes[1, 1].text(h_freq / 1e6, prs_spec[freq_mask].max() / 1e3 * 0.9,
                        f" {h+1}f", fontsize=9, color="red", va="top")
