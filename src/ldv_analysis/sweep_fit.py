...
def detect_channel_geometry(pos_x, pos_y, rssi, pressure_abs, hw) -> Geom:
    ...
    if rssi is not None:
        ...
        res = brute(cost, ranges=((c_lo, c_hi), (c_lo, c_hi)), Ns=100, finish=fmin)
    else:
        ...
        res = brute(cost, ranges=((c_lo, c_hi), (c_lo, c_hi)), Ns=100, finish=None)
...
def fit_complex_column(col_c, width_grid, channel_width, harmonic):
    ...
    p0, clip = _project(col_c[finite], mode[finite], sigma_clip=3.0, min_keep_frac=0.5)
    pred = p0 * mode[finite]
...
def fit_axial(
    cache, valid, channel_width, *, geom: Geom | None = None
    ...
    if is_1d:
        ...
        fit = AxialFit(
            ...
            is_1d=True,
            center=r1.center,
            y_c=pos[valid] - r1.center,
            p=P1_abs[valid],
            phase_deg=P1_ph[valid],
            p1_n2_mag=abs(r1_n2.p0),
            p1_n2_complex=r1_n2.p0,
            r2_p1_n2=r1_n2.r2,
        )
        return fit, geom
    ...
    p0_y1 = fit_columns(grid_mag1, cg.width_grid, channel_width, harmonic=1, sigma_clip=3.0)
    ...
    if has_2f:
        ...
        p0_y2 = fit_columns(grid_mag2, cg.width_grid, channel_width, harmonic=2, sigma_clip=3.0)
    ...
    p0_y1_n2 = fit_columns(grid_mag1, cg.width_grid, channel_width, harmonic=2, sigma_clip=3.0)
    if np.all(np.isnan(p0_y1_n2)):
        ...
        p1_n2_c, r2_p1_n2 = fit_complex_column(col1_n2, cg.width_grid, channel_width, 2)
    ...
    fit = AxialFit(
        ...
        p2_complex=p2c,
        r2_2=r2_2,
        x_best_mm=float(cg.length_grid[best] * 1e3),
        is_1d=False,
        center=0.0,
        y_c=cg.width_grid,
        p=grid_mag1[:, best],
...
def sweep_peaks(run_dir: Path, channel_width: float, cache_dir: Path) -> SweepPeaks:
    ...
    *cache_dir* for the shared FFT cache so repeated runs are fast.
    ...
    for p in files:
        ...
        # Drive frequency from the filename (f<Hz>.h5) keeps this cache-only;
        # opening the .h5 for drive_frequency_hz_nominal would hydrate the
        # entire file on online-only (OneDrive Files-On-Demand) stores.
        try:
            f_nom = float(p.stem.lstrip("f"))
        except ValueError:
            f_nom = float(c["f_drive"])
    ...
    return SweepPeaks(
        ...
        r2_1=r2a,
        x_best_mm=xba,
        peak_p1_kpa=float(p1a[i1]),
        peak_p1_freq_mhz=float(fm[i1]),
        peak_p2_kpa=float(p2a[i2]),
        peak_p2_freq_mhz=float(fm[i2]),
        is_1d=is_1d,
    )
