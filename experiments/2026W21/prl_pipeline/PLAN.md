...
| R2 | Phase response of Ch2 (velocity decoder) — Polytec-spec assumption | The pipeline derives every published `P_nf` from Ch2 (velocity decoder), inverted via `p = −v/(2π f H · dn/dp)`. Complex / phase-dependent claims (Fig 2b `K_exp` sign, Fig S2 complex residues) rely on the Polytec velocity decoder's phase response being flat in our 1.9–11 MHz band, per the vendor spec (typically < 1° phase distortion in the linear band). **No in-band Ch2 phase audit has been performed.** A Ch3 (displacement) cross-check was attempted (`figS1_decoder_check.py`) and failed by a 6× magnitude factor + 8° phase offset, but this only constrains Ch3's calibration — not Ch2's — and Ch3 is not used anywhere in the publishing pipeline. The original R2 framing (audit of complex `C_{p←d}(f)`) does **not** apply to a Ch2-only pipeline. Action: trust the Polytec spec for Ch2; if a reviewer questions Ch2 phase, run an independent in-band phase test (e.g., needle hydrophone, or air-load mechanical-impedance phase check). | ldv-analysis | **documentation note** (assumption explicitly recorded; not gating any published number) |
| R3 | 4f / 5f adoption sanity check on W21 SNR | Inspect actual SNR and `E_n/E_1` uncertainty at top drives against the contract's adoption thresholds (SNR > 5 AND `E_n/E_1` above the 1σ band). Record per-run per-harmonic adoption booleans in `fig3.json` metadata, with `adoption_reason ∈ {adopted, snr_below_threshold, e_ratio_below_1sigma}`. Manuscript implication: the `E_foc / E_1 = 1 − 4 E_2/E_1 + 9 E_3/E_1 − 16 E_4/E_1 + 25 E_5/E_1 − ⋯` series in the contract Fig. 3 section truncates wherever the adoption flag goes false. If 4f/5f are never adopted at any drive, drop them from the series in the contract and the manuscript draft. | ldv-analysis | **open** (WP-L4 acceptance gate; manuscript-side update is mechanical once the data answer is known) |
